...
class LikesService {
  ...
  async addLikeAlbumByid({ userId, albumId }) {
    ...
    const { rowCount } = await this._pool.query(checkQuery);
    ...
    const { rows, rowCount: insertRowCount } = await this._pool.query(insertQuery);
  ...
  async getLikeAlbumById(albumId) {
    try {
      ...
      const likeCount = parseInt(rows[0].count, 10);
